...
對所有 Grok 生成請求，節奏控制是**硬性規則**，不是建議。

### 5.1 必須遵守

- 禁止並發批量生成
- 必須串行，一次只打一張圖 / 一個視頻
- 每次生成成功後，必須加入**隨機間隔**再發下一次請求
- 若出現限流、通道不足、上游不穩，必須立即退避，不能硬撞

### 5.2 建議間隔

- 正常情況：隨機等待 **8–25 秒**
- 若上一張耗時長、網路抖動、上游狀態差：隨機等待 **20–45 秒**
- 若出現 `429` / `503` / `No available tokens` / `rate_limit_exceeded` / `無可用渠道`：退避 **60–180 秒**

### 5.3 執行原則

- 每生成 1 個結果，先確認文件或 URL 已拿到，再進行下一次
- 大批量 prompt 必須拆成多輪執行，不要一口氣連發
- 同一後端連續失敗 2 次，立即停止，不要繼續撞
- 優先保留已成功結果，不要為了補一張圖全量重跑
...
- 正常情況下，視頻結果應返回服務自身的 `/v1/files/video/...` 鏈接；如果仍看到 `assets.grok.com` 直鏈，說明服務端回源/部署有問題，應優先修服務端
- 若鑑權通過但上游 token 未配置，請求可能在業務層失敗；這不是 CLI 鑑權問題
- 圖片 / 視頻若連發，極易觸發限流或通道耗盡；節奏控制不是可選項
